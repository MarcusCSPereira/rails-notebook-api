module V2
  class ContactSerializer < ActiveModel::Serializer
    ...
    belongs_to :kind, optional: true do
      link(:self) { v2_contact_relationships_kind_url(object.id) }
      link(:related) { v2_contact_kind_url(object.id) }
    end

    has_many :phones do
      link(:self) { v2_contact_relationships_phones_url(object.id) }
      link(:related) { v2_contact_phones_url(object.id) }
    end

    has_one :address, inverse_of: :contact do
      link(:self) { v2_contact_relationships_address_url(object.id) }
